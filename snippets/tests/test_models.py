...
class DeviceModelTest(TestCase):
    ...
    def test_mac_label(self):
        print("setUpTestData: Data Set-Up for Device Model - Check Label Name - mac")
        device = Device.objects.get(id=1)
        field_label = device._meta.get_field('mac').verbose_name
        self.assertEqual(field_label, 'mac')

    def test_mac_max_length(self):
        print("setUpTestData: Data Set-Up for Device Model - Check Max Length - mac")
        device = Device.objects.get(id=1)
        max_length = device._meta.get_field('mac').max_length
        self.assertEqual(max_length, 150)

    def test_object_name(self):
        print("setUpTestData: Data Set-Up for Device Model - __str__ function to get mac")
        device = Device.objects.get(id=1)
...
class DeviceLogModelTest(TestCase):
    ...
    def test_value_label(self):
        print("setUpTestData: Data Set-Up for Device Model - Check Label Name - value")
    ...
    def test_object_name(self):
        print("setUpTestData: Data Set-Up for Device Model - __str__ function to get value")
        device_val = DeviceLog.objects.get(id=1)
        expected_object_name = device_val.value
